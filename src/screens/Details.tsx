...
export function Details() {
  ...
  function handleOrderClosed(){
    if(!solution){
      return Alert.alert('Solicitação', 'Informe a solução do problema para encerrar a solicitação.')
    }
  ...
  return (
    <VStack flex={1} bg="gray.700">
      ...
      <HStack bg="gray.500" justifyContent="center" p={4}>
        ...
        <Text fontSize="sm" ml={2} textTransform="uppercase"
      ...
      <ScrollView mx={5} showsVerticalScrollIndicator={false}>
        ...
        <CardDetails title="descrição do problema" description={order.description} icon={ClipboardText} 
        ...
        <CardDetails title="solução" description={order.solution} icon={CircleWavyCheck} footer={order.closed && `Encerrado em: ${order.closed}`}>
          {
            order.status === 'open' &&
            <Input placeholder="Descrição da solução" onChangeText={setSolution} h={24} textAlignVertical="top" multiline />
          }
        </CardDetails>
      </ScrollView>
      {
